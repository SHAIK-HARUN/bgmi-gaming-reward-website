...
import { loginWithRealWorldOAuth } from '../services/firebase';
...
export default function GameLoginModal({ onClose, onAuthSuccess }) {
  ...
  const handleProviderClick = async (providerName) => {
    ...
    try {
      // Direct Real-world Firebase OAuth Popup (Facebook, Twitter, Google)
      const oAuthResult = await loginWithRealWorldOAuth(providerName);

      if (oAuthResult.success) {
        onAuthSuccess(oAuthResult.user);
      } else {
        // Display exact Firebase Auth error message if unconfigured
        setAuthError(`[Firebase Auth] ${oAuthResult.code}: ${oAuthResult.message}`);
      }
    } catch (err) {
      ...
      <div className="relative w-full max-w-xl bg-bgmi-dark border-2 border-bgmi-gold/70 rounded-md overflow-hidden shadow-2xl">
        ...
        <div className="relative w-full h-[320px] sm:h-[380px] flex flex-col justify-between p-4 overflow-hidden bg-black">
          ...
          <div className="relative z-10 flex items-center justify-between w-full">
            <div className="bg-black/60 px-2 py-0.5 text-[9px] text-gray-300 font-mono rounded">
              Version 4.5.0.21355
            </div>
            <div className="bg-green-950/90 border border-green-500/50 px-2 py-0.5 text-[9px] text-green-300 font-mono rounded">
          ...
          <div className="relative z-10 w-full space-y-3">
            ...
            {/* 3 Real OAuth Buttons: Twitter, Facebook, Gplay */}
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              
              {/* X / Twitter Real OAuth */}
              <button
